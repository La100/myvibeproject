...
- `CHATKIT_SELF_HOSTED_SERVER_URL=http://<your-private-chatkit-service>`
- `CHATKIT_SELF_HOSTED_INTERNAL_SECRET=<shared-secret>`

Plus your existing app variables such as Stripe, R2, Resend, and OpenAI.

## Railway service 2: self-hosted ChatKit backend

This repo does not yet include the backend implementation itself. Per the OpenAI advanced integration docs, this is a separate ChatKit API service. Railway should run it as its own service and keep it on private networking if possible.
...
- `CHATKIT_SELF_HOSTED_INTERNAL_SECRET=<same-shared-secret-as-the-app>`
